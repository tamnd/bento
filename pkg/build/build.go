...
func Build(opts Options) (string, error) {
	...
	output := opts.Output
	if output == "" {
		base := cpath.Base(entry)
		output = strings.TrimSuffix(base, cpath.Ext(base))
	}
...
func EmitGoWithOptions(entry, stamp string, opts EmitOptions) (string, error) {
	abs, err := cpath.Abs(entry)
	if err != nil {
		return "", fmt.Errorf("bento build: %s: %w", entry, err)
	}
	if _, err := os.Stat(cpath.ToOS(abs)); err != nil {
		return "", fmt.Errorf("bento build: %s: %w", entry, err)
	}
	...
	// The header names the entry by its base name, which is the same string on
	// either platform once the path has been through cpath, so a golden generated
	// on Windows and one generated on Unix are byte-identical.
	header := fmt.Sprintf("// Code generated by bento %s from %s. DO NOT EDIT.\n\n", stamp, cpath.Base(abs))
...
//
// The symlink walk is a disk operation, so it takes an operating system path and
// gives one back, and both ends convert. Without the conversion back, a Windows
// build would compare the loader's "C:/x/main.ts" against EvalSymlinks's
// "C:\x\main.ts" and find no entry among its own source files.
func canonicalPath(p string) string {
	if resolved, err := filepath.EvalSymlinks(cpath.ToOS(p)); err == nil {
		return cpath.FromOS(resolved)
	}
	return cpath.FromOS(p)
}
...
func link(source, output string, cgo bool) error {
	...
	// output is a checker path; the go toolchain is an external tool, so it gets the
	// operating system spelling. Everything else in here is already an OS path: the
	// scratch directory came from MkdirTemp and the module root from the executable.
	cmd := exec.Command("go", "build", "-o", cpath.ToOS(output), ".")
	cmd.Dir = dir
	enabled := "0"
	if cgo {
		enabled = "1"
