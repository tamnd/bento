...
func (r *Renderer) propertyAccess(n frontend.Node) (ast.Expr, error) {
	...
	// way a same-module `const f = inc` does, and m.K reads the package-level Go var an
	// exported const or let composed to (#589). A member with no Go value behind it, a
	// class export or a non-literal const the composition does not materialize, or an
	// overloaded, generic, or defaulting export whose bare name reference cannot stand
	...
	if r.isDynamic(obj) {
		...
		r.requireImport(valuePkg)
		// A read of the legacy __proto__ accessor is the object's prototype, unless the
		// receiver carries an own "__proto__" data property (JSON.parse installs one),
		// which shadows the inherited accessor. ProtoRead resolves that precedence: own
		// data property first, else the prototype. The write side mirrors the accessor
		// through SetPrototype.
		if prop == "__proto__" {
			return &ast.CallExpr{Fun: &ast.SelectorExpr{X: recv, Sel: ident("ProtoRead")}}, nil
		}
