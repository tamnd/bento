...
// This file lowers numeric enums (05_type_lowering, the enums section). A plain
// numeric enum becomes a float64-backed constant per member, so a member read
// A.B lowers to the constant AB and every use of an enum value rides the number
// path unchanged: the checker types an enum member as number, and typeExpr maps
// an enum-typed slot to float64, so a comparison, a switch, a binding, and a
// console print all see the float64 the value already is. A named Go type would
// fight that float64 floor and buy nothing the checker has not already enforced,
// so the representation is the bare constants.
//
// A const enum emits no type and no constants: each member read inlines the
// member's numeric value at the use site, the erasure TypeScript itself performs.
//
// String enums (the tag-plus-table form) and computed members are a later slice
// and hand back at registration, routing the unit to the engine rather than
// lowering a shape this file does not build.

// enumInfo is one registered numeric enum. A const enum keeps its members for the
// inline lookup but emits nothing at package level; a plain enum emits a const
// block and each member read names the member's Go constant.
type enumInfo struct {
	name    string // the TypeScript enum name
	goName  string // the exported Go name the member constants are prefixed with
	isConst bool
	members []enumMember
}

// enumMember is one member of a numeric enum: its source name, the Go constant
// name a plain enum's read resolves to (goName + the exported member name), and
// the float64 value the member holds.
...
func (r *Renderer) registerEnum(decl frontend.Node, taken map[string]bool) error {
	...
	info := &enumInfo{name: name, goName: goName, isConst: isConst}
	next := 0.0
	...
	for _, memberNode := range kids[1:] {
		...
		if len(mkids) >= 2 {
			...
			// one (the common negative-sentinel form), is folded to its value. A string
			// literal (a string enum) or any other computed expression is a later slice,
			// so the whole enum hands back.
		...
		if minted[goConst] {
			return &NotYetLowerable{Reason: "enum " + name + " members collide on the Go constant " + goConst + ", a later slice"}
		}
		// A plain enum names package-level constants; a const enum inlines and
		// speaks no names, so only a plain enum checks the constant against the
		// module's existing identifiers.
		if !isConst && taken[goConst] {
			return &NotYetLowerable{Reason: "the module already speaks " + goConst + ", the name enum " + name + "." + mName + " needs"}
		}
		minted[goConst] = true
		info.members = append(info.members, enumMember{name: mName, goConst: goConst, value: val})
	}
...
// name map. An unregistered enum (a string enum, deferred) reports false.
...
// renderEnums emits the package-level const block for each plain numeric enum in
// source order. A const enum emits nothing: its members were inlined at every use
// site. Each block reads like a hand-written Go enum, a float64-typed constant per
// member:
//
//	const (
//		ColorRed   float64 = 0
